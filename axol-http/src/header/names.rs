...
/// A string that will match others regardless of ascii case
#[derive(Clone, Copy, Debug)]
struct CaselessStr<'a>(&'a str);

impl<'a, 'b> PartialEq<CaselessStr<'b>> for CaselessStr<'a> {
    fn eq(&self, other: &CaselessStr<'b>) -> bool {
        self.0.eq_ignore_ascii_case(other.0)
    }
}

impl<'a, 'b> Eq for CaselessStr<'a> {}
    ...
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        for i in 0..bytes.len() {
            if bytes[i] >= b'A' && bytes[i] <= b'Z' {
                if start != i {
                    state.write(&bytes[start..i]);
                }
                start = i + 1;
                state.write(&[bytes[i] + b' ']);
            }
        }
        if start != bytes.len() {
...
macro_rules! standard_headers {
    (
        ...
        $(
            $(#[$docs])*
            pub const $upcase: &'static str = $name_bytes;
        )+
        const ALL_HEADERS: &[&'static str] = &[
            $(
                $upcase,
            )+
        ];
...
lazy_static::lazy_static! {
    static ref HEADER_MAP: HashSet<CaselessStr<'static>> = {
        let mut out = HashSet::new();
        for header in ALL_HEADERS {
            out.insert(CaselessStr(*header));
        }
        out
    };
}

/// If found, returns a static reference to an ascii lowercase version of `name`.
pub(super) fn maybe_static_lowercase<'a>(name: &'a str) -> Option<&'static str> {
    let value: Option<&CaselessStr<'a>> = HEADER_MAP.get(&CaselessStr(name));
    match value {
        Some(x) => {
            let x: &'a str = (*x).0;
            // SAFETY: this is safe because the underlying value in the hashset is 'static, but weird type coercion
            // and inability to implement borrow (cannot state 'a: ! 'static) yields to this
            let x: &'static str = unsafe { std::mem::transmute(x) };
            Some(x)
        }
        None => None,
    }
...
mod tests {
    ...
    use super::CaselessStr;

    #[test]
    fn test_caseless_str() {
        assert_eq!(CaselessStr("test"), CaselessStr("TEST"));
        let mut set = HashSet::new();
        set.insert(CaselessStr("tESt"));
        assert!(set.contains(&CaselessStr("test")));
        assert!(set.contains(&CaselessStr("TEST")));
        assert!(set.contains(&CaselessStr("tESt")));
        assert!(set.replace(CaselessStr("test")).is_some());
        assert!(set.contains(&CaselessStr("TEST")));
        assert_eq!(set.len(), 1);
        assert_eq!(set.get(&CaselessStr("Test")).map(|x| x.0), Some("test"))
    }
}
