...
pub enum PathDeserializationError {
    ...
    #[error("Wrong number of path arguments for `Path`. Expected {expected} but got {got}. Note that multiple parameters must be extracted with a tuple `Path<(_, _)>` or a struct `Path<YourParams>`")]
    ...
    #[error("Cannot parse `{key}` with value `{value:?}` to a `{expected_type}`")]
    ParseErrorAtKey {
    ...
    #[error("Cannot parse value at index {index} with value `{value:?}` to a `{expected_type}`")]
    ParseErrorAtIndex {
    ...
    #[error("Cannot parse `{value:?}` to a `{expected_type}`")]
    ParseError {
        /// The value from the URI.
        value: String,
        /// The expected type of the value.
...
impl PathDeserializationError {
    fn wrong_number_of_parameters() -> WrongNumberOfParameters<()> {
        WrongNumberOfParameters { got: () }
    }
...
mod tests {
        ...
        V: AsRef<str>,
    {
        values
            .into_iter()
            .map(|(k, v)| (Arc::from(k.as_ref()), v.as_ref().to_string()))
            .collect()
    }
    ...
    fn test_parse_single_value() {
        ...
        let error_kind = i32::deserialize(PathDeserializer::new(&url_params)).unwrap_err();
        assert!(matches!(
            error_kind,
            PathDeserializationError::WrongNumberOfParameters {
                expected: 1,
    ...
    macro_rules! test_parse_error {
        (
            ...
            let actual_error_kind =
                <$ty>::deserialize(PathDeserializer::new(&url_params)).unwrap_err();
    ...
    fn test_parse_seq_tuple_unsupported_key_type() {
        test_parse_error!(
            vec![("a", "false")],
            Vec<(u32, String)>,
            PathDeserializationError::Message("Unexpected key type".to_owned())
        );
    }
