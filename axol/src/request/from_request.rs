use axol_http::{request::RequestPartsRef, Body, Extensions};
...
impl<'a, T: FromRequest<'a>> FromRequest<'a> for Option<T> {
    async fn from_request(request: RequestPartsRef<'a>, extensions: &mut Extensions, body: Body) -> Result<Self> {
        Ok(T::from_request(request, extensions, body).await.ok())
    }
}
...
impl<'a, T: FromRequest<'a>> FromRequest<'a> for Result<T> {
    async fn from_request(request: RequestPartsRef<'a>, extensions: &mut Extensions, body: Body) -> Result<Self> {
        Ok(T::from_request(request, extensions, body).await)
    }
}

#[async_trait::async_trait]
impl<'a> FromRequest<'a> for Body {
    async fn from_request(_: RequestPartsRef<'a>, _: &mut Extensions, body: Body) -> Result<Self> {
        Ok(body)
    }
}
...
macro_rules! impl_from_request {
    (
            ...
            $last: FromRequest<'a>,
        {
            async fn from_request(request: RequestPartsRef<'a>, extensions: &mut Extensions, body: Body) -> Result<Self> {
                $(
                    let $ty = $ty::from_request_parts(request, extensions).await?;
                )*

                let $last = $last::from_request(request, extensions, body).await?;
